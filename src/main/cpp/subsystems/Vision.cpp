...
double Vision::GetTargetAngleFromDistance(double distance)
{
    if(distance == 0.0) // Protect against zero distance and division by zero
        return 0.0;
    return RadiansToDegrees(std::atan((TargetHeight - CameraHeight) / distance)) - CameraAngleDegrees;  // Do some trigonometry to compute the angle that corresponds to the input distance
}

// Adapted from http://docs.limelightvision.io/en/latest/cs_estimating_distance.html
double Vision::GetTargetDistanceFromAngle(double angle)
{
    auto    tangent{ std::tan(DegreesToRadians(angle + CameraAngleDegrees)) };
